...
var Masher = function(config, debug) {
  ...
  //this.config = aug(true, {}, defaults, config);
  this.config = config;
  
  this.config.assetPath = path.join(this.root, this.config.assetPath);
  this.config.outputPath = path.join(this.root, this.config.outputPath);
  this.debug = debug || (process.env.NODE_ENV != 'production');
};
...
Masher.prototype.build = function(callback) {
  ...
  var queue = res.queue(function(key, next) {
    self.buildGroup(key, self.config.groups[key], function(group) {
      console.log(group);
      self.config.groups[key].styleOut = group.style.filename.replace(self.config.assetPath, '');
      self.config.groups[key].scriptOut = group.script.filename.replace(self.config.assetPath, '');
      next();
    });
  });
    ...
    addScript: function(groupName) {
      ...
      if (self.debug) {
        ...
        for (var i = 0, c = scripts.length; i < c; i++) {
          ...
          out.push(strf(format, scriptsPath+'/'+script));
        }
        return out.join('\n');
      } else {
    ...
    addStyle: function(groupName) {
      ...
      if (self.debug) {
        ...
        for (var i = 0, c = styles.length; i < c; i++) {
          ...
          out.push(strf(format, stylesPath+'/'+style));
        }
        return out.join('\n');
      } else {
